...
from glamcheck.models.composition.domain.component_model import ComponentModel
from glamcheck.services.abstract_component_service import AbstractComponentService
import aiohttp


class CosmobaseComponentService(AbstractComponentService):
    def __init__(self):
        self.__session = aiohttp.ClientSession(base_url="https://cosmobase.ru/handbook")

    async def shutdown(self) -> None:
        await self.__session.close()

    def find_component(self, title: str) -> ComponentModel:
        self.__parse_component_links(title)

    async def __parse_component_links(self, search: str):
...
'hbSearch=%D0%B2%D0%B0&hbGroup=0&hlbFrom=&hlbTo=&factor=&hbEffects=0&hbSkin=0&hbNatural=false&lookDesc=false&start=0&searchType=standart'

        response = await self.__session.post(url="/search", headers=headers, data=data)
