...
def parse_categories(soup: bs4.BeautifulSoup) -> list[str]:
    ...
    label_tag: Optional[bs4.Tag] = soup.find(lambda tag: tag.name == "label" and "категориям" in tag.text)
...
def parse_cosmetic_properties(soup: bs4.BeautifulSoup) -> list[CosmeticProperty]:
    # Add exceptions handling

    label_tag: Optional[bs4.Tag] = soup.find(lambda tag: tag.name == "label" and "Косметические" in tag.text)
    ...
    for t in cosmetic_properties_tag.findAll('div'):
        ...
        cosmetic_properties.append(CosmeticProperty(
...
def parse_additional_properties(soup: bs4.BeautifulSoup) -> list[AdditionalProperty]:
    # Add exceptions handling

    label_tag: Optional[bs4.Tag] = soup.find(lambda tag: tag.name == "label" and "Дополнительные" in tag.text)
...
def parse_aliases(soup: bs4.BeautifulSoup) -> list[str]:
    ...
    label_tag: Optional[bs4.Tag] = soup.find(lambda tag: tag.name == "label" and "Синонимы" in tag.text)
...
def parse_danger_factor(soup: bs4.BeautifulSoup) -> Optional[DangerFactor]:
    # Add exceptions handling

    label_tag: Optional[bs4.Tag] = soup.find(lambda tag: tag.name == "label" and "Фактор опасности" in tag.text)
...
def parse_naturalness(soup: bs4.BeautifulSoup) -> Optional[Naturalness]:
    # Add exceptions handling

    label_tag: Optional[bs4.Tag] = soup.find(lambda tag: tag.name == "label" and "Натуральность" in tag.text)
    ...
    return Naturalness.SYNTHETIC if naturalness_value == 'Синтетический' else Naturalness.NATURAL


def parse_comment(soup: bs4.BeautifulSoup) -> Optional[str]:
    # Add exceptions handling

    label_tag: Optional[bs4.Tag] = soup.find(lambda tag: tag.name == "label" and "Комментарий безопасности" in tag.text)
...
def parse(content: str, url: str) -> Component:
    # Add exceptions handling
